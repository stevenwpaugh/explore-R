...
pdf(file="20120109_ETP_WGS_RECURR_cnv_corr.pdf", width=20, height=20)
##############################


cnvdata <- read.csv("tall_wgs_cna_20120105.cnv", as.is=TRUE, stringsAsFactors=FALSE)
cnvdata <- rbind (cnvdata, 
		c("SJTALL008", 1, 1, 2, 0))
...
#cnvdata$chr <- cnvdata$chromosome
...
cnvdata$patientnum <- as.factor(cnvdata$patientnum)
cnvdata$patientnum <- as.numeric(cnvdata$patientnum)
cnvdata$patientnum <- cnvdata$patientnum + max (wgscnvdata$patientnum)
...
cnvdata <- rbind (wgscnvdata, cnvdata)
