...
pdf(file="20120108_RB_WGS_plus_addition_cnv.pdf", width=20, height=3)

cnvdata <- read.csv("CNV_CN_20120106", as.is=TRUE, stringsAsFactors=FALSE)
cnvdata$copy_number <- as.numeric(cnvdata$copy_number_change)
cnvdata$copy_number_change <- NULL


cnvdata <- rbind (cnvdata, c("SJRB002", 1, 1, 2, 0))#added so RB002 shows

###################
##Additional data##
###################
cnvdata <- rbind (cnvdata, c("SJRB002", 6, 1, 60500000, 1))
###################
...
rect (cnvdata[i,"genomic.start"], cnvdata[i,"patientnum"]-0.5, cnvdata[i,"genomic.end"], cnvdata[i,"patientnum"]+0.5, col=cnvdata[i,"col"], border=cnvdata[i,"col"], ljoin=1)
}
...
par(fig=c(0.02, 0.15, 0.02, 0.25), new=T)
